...
func main() {
	...
	case "list":
		var path path
		collection.ForEach(func(i int, data []byte) bool {
	...
	case "add":
		...
		for _, p := range args[1:] {
			...
			if err := collection.InsertJSON(path); err != nil {
				fatalErr = err
				return
			}
			fmt.Printf("+ %s\n", path)
	...
	case "remove":
		var path path
		collection.RemoveEach(func(i int, data []byte) (bool, bool) {
			err := json.Unmarshal(data, &path)
			if err != nil {
				fatalErr = err
				return false, true
